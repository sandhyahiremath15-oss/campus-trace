  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (error.code === 'auth/popup-closed-by-user') {
        ...
        message = "This domain is not authorized. Add it in the Firebase Console Settings.";
      }
  ...
  const handleEmailSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error.code === 'auth/user-not-found' || error.code === 'auth/wrong-password') {
        message = "Invalid email or password. Please ensure you have registered this account.";
      }
      toast({
        variant: "destructive",
        title: "Sign In Failed",
      ...
      <div className="w-full max-w-[440px] bg-white rounded-[24px] p-6 md:p-10 shadow-xl shadow-slate-200/50 border border-slate-100 animate-in fade-in zoom-in-95 duration-500">
        ...
        <form onSubmit={handleEmailSignIn} className="space-y-4">
          ...
          <div className="flex flex-col sm:flex-row-reverse items-center justify-between gap-4 pt-4">
            ...
            <Link href="/auth/register" className="text-sm font-bold text-blue-600 hover:text-blue-700 transition-colors">
              Create account
            </Link>
          </div>
